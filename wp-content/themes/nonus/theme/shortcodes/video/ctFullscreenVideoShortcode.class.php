...
class ctFullscreenVideoShortcode extends ctShortcode {
	...
	public function handle($atts, $content = null) {
		...
		$imgBag = $imgsrc?' style="background-image:url('.esc_attr($imgsrc).')"':'';

		return '<div id="' . $id . '" class="height100 videoContainer"'.$imgBag.'>' . do_shortcode($content . $this->embedShortcode('video', $atts)) . '</div>';
	}
