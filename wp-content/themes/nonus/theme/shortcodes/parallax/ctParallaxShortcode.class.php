...
class ctParallaxShortcode extends ctShortcode {
	...
	public function handle($atts, $content = null) {
		...
		if($type == 'main'){
			return do_shortcode('<header id="MainHeader" class="main-header parallax ' . $class . ' '. $darktextHtml. '" style="background-image: url(' . $imgsrc . ')">
							            ' . $content .'
								</header>');
		}else{
			return do_shortcode('<section' . $id . ' class="parallax ' . $class . ' '. $darktextHtml. '" style="background-image: url(' . $imgsrc . ')">
							            ' . $content .'
								</section>');
		}
